...
int ASocket::SelectSocket(const ASocket::Socket sd, const size_t msec)
{
   ...
   if (msec > 0)
   {
      tval.tv_sec = static_cast<long>(msec / 1000);
      tval.tv_usec = static_cast<long>((msec % 1000) * 1000);
      tvalptr = &tval;
   }
   ...
   int sockFd = static_cast<int>(sd + 1);
   res = select(sockFd, &rset, nullptr, nullptr, tvalptr);
                           ...
                           const size_t msec, size_t& selectedIndex)
{
   ...
   for (size_t i = 0; i < count; i++)
   {
      ...
      {
         max_fd =static_cast<int>(pSocketsToSelect[i]);
      }
   }
