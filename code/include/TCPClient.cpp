...
bool CTCPClient::Send(const char* pData, const size_t uSize) const
{
   ...
   do
   {
      const int flags = 0;
      int nSent = 0;
      nSent = send(static_cast<int>(m_ConnectSocket), pData + total, static_cast<int>(uSize - total), flags);
...
int CTCPClient::Receive(char* pData, const size_t uSize, bool bReadFully /*= true*/) const
{
   ...
   do
   {
      int nRecvd = recv(static_cast<int>(m_ConnectSocket), pData + total, static_cast<int>(uSize - total), 0);
