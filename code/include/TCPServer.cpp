...
int CTCPServer::Receive(const CTCPServer::Socket ClientSocket,
	...
	int total = 0;
	do {
		int nRecvd = recv(static_cast<int>(ClientSocket), pData + total, static_cast<int>(uSize - total), 0);
...
bool CTCPServer::Send(const Socket ClientSocket, const char* pData, size_t uSize) const {
	...
	do {
		...
		nSent = send(static_cast<int>(ClientSocket), pData + total, static_cast<int>(uSize - total), flags);
